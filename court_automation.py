...
CONFIG = {
    ...
    "email": {
        ...
        "recipient_emails": ["ami@apmh.in","hemant@apmh.in"],
    },
...
async def run_nclt_automation(search_date: datetime = None):
    ...
    # Save results for the UI to display
    nclt_ui_entries = []
    for e in all_matches:
        nclt_ui_entries.append({
            "source":  "NCLT",
            "client":  e.get("client_name", ""),
            "court":   e.get("_pdf_court", ""),
            "case_no": e.get("cp_no", "") or e.get("ia_ca_no", ""),
...
# ── Results file writer (used by all courts) ──────────────────────────────────
def _save_results(entries: list, run_date: str, source_label: str):
    """
    Write / merge results into last_results.json so the UI can display them.
    Called after each court run. Merges with any existing results from the
    same run (so NCLT + NCLAT + SCI results all appear together).
    """
    ...
    if results_path.exists():
        try:
            ...
            # If the date changed, start fresh
    ...
    # Remove old entries from this source so re-runs don't duplicate
    all_entries = [e for e in all_entries if e.get("source") != source_label]
    ...
    try:
        ...
        log.info("[results] Saved %d entr(ies) to %s", len(all_entries), results_path.name)
    except Exception as e:
        log.warning("[results] Could not write last_results.json: %s", e)
