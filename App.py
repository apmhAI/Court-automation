...
with tab_results:
    ...
    if rr_check and rr_check.get("status") in ("pending", "running"):
        ...
        # results date is now stored as DD-MM-YYYY; also handle legacy "DD Month YYYY"
        results_date_norm = results_date_raw
        for _fmt in ("%d-%m-%Y", "%d %B %Y", "%d/%m/%Y"):
            try:
                results_date_norm = datetime.strptime(results_date_raw, _fmt).strftime("%d-%m-%Y")
                break
            except Exception:
                pass
    ...
    if not entries:
        ...
        # Convert stored DD-MM-YYYY to human-readable for display
        run_date_display = run_date_r
        for _fmt in ("%d-%m-%Y", "%d/%m/%Y"):
            try:
                run_date_display = datetime.strptime(run_date_r, _fmt).strftime("%d %B %Y")
                break
            except Exception:
                pass

        st.caption(
            f"**{len(entries)} match(es)** for **{run_date_display}** &nbsp;·&nbsp; "
        ...
        st.download_button(
            "⬇ Download CSV",
            data=csv,
            file_name=f"court_results_{run_date_r.replace(' ','_').replace('/','_').replace('-','_')}.csv",
            mime="text/csv",
        )
