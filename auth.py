...
def _send_reset_email(to_email: str, otp: str, name: str) -> bool:
    """Send OTP via the same SMTP config used by court_automation.py."""
    try:
...
def request_password_reset(email: str) -> tuple:
    ...
    sent = _send_reset_email(key, otp, users[key]["name"])
    if not sent:
        return False, "Could not send email. Check SMTP settings in Streamlit secrets."
    return True, "ok"
